...
void sched_change_priority(int id,char prior){
	...
	while ((curr != NULL)&&(flag == 0))
	{
		if (curr->id == id){
			stop = first->pid;
			if (mid == curr) mid = prev;
			if (first == curr) first = curr->next;
			if (last == curr) last = prev;
			if (prev != NULL) prev->next = curr->next;
			enqueue(curr->id, curr->pid, curr->name, prior);
			free(curr);
			flag = 1;
		}
		else{
...
void sched_low_task(int id){
	...
	while ((curr != NULL)&&(flag == 0))
	{
		if (curr->id == id){
			...
			if (last == curr) last = prev;
			if (prev != NULL) prev->next = curr->next;
			enqueue(curr->id, curr->pid, curr->name, 'l');
			free(curr);
			flag = 1;
		}
		else{
...
sched_create_task(char *executable)
{
	...
	++tasknum;
	printf("tasknum is %d \n", tasknum);
	enqueue(tasknum, pid, executable, 'l');
...
process_request(struct request_struct *rq)
{
	switch (rq->request_no) {
		...
		case REQ_EXEC_TASK:
			...
			return 0;
		case REQ_HIGH_TASK:
			sched_change_priority(rq->task_arg,'h');
			return 0;
		case REQ_LOW_TASK:
			sched_change_priority(rq->task_arg,'l');
			return 0;
		default:
			return -ENOSYS;
	}
